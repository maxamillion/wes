...
class CredentialMonitor(QObject):
    ...
    def __init__(self, config_manager: ConfigManager, monitoring_config: Optional[MonitoringConfig] = None):
        super().__init__()
    ...
    def start_monitoring(self):
        ...
        self.logger.info(f"Credential monitoring started (interval: {self.monitoring_config.check_interval_minutes}m)")
    ...
    async def _check_credential(self, service: str, credential_type: str):
        ...
        if status_key not in self.credential_statuses:
            self.credential_statuses[status_key] = CredentialStatus(
                ...
                next_check=datetime.now() + timedelta(minutes=self.monitoring_config.check_interval_minutes)
            )
        ...
        try:
            ...
            if expires_at:
                ...
                if days_until_expiry <= self.monitoring_config.expiration_warning_days:
                    self.credential_expiring.emit(service, credential_type, days_until_expiry)
            ...
            if previous_health != status.healthy:
                self.credential_status_changed.emit(service, credential_type, status.healthy)
            
            # Handle consecutive failures
            if status.error_count >= self.monitoring_config.max_consecutive_failures:
                self.credential_failed.emit(service, credential_type, status.last_error or "Multiple consecutive failures")
            
            # Schedule next check
            status.next_check = datetime.now() + timedelta(minutes=self.monitoring_config.check_interval_minutes)
            ...
            self.logger.error(f"Credential check failed for {service}:{credential_type}: {e}")
            
            if previous_health != status.healthy:
                self.credential_status_changed.emit(service, credential_type, status.healthy)
    
    async def _perform_health_check(self, service: str, credentials: Dict[str, str]) -> Dict[str, Any]:
        ...
        try:
            if service == "jira":
                ...
                success, message = self.validator.validate_google_credentials(credentials)
    ...
    async def _attempt_auto_refresh(self, service: str, credential_type: str):
        ...
        if service in self.refresh_handlers:
            try:
                ...
                self.logger.error(f"Auto-refresh failed for {service}:{credential_type}: {e}")
    ...
    def _get_credentials_for_service(self, service: str) -> Optional[Dict[str, str]]:
        ...
        try:
            if service == "jira":
                ...
                refresh_token = self.config_manager.retrieve_credential("google", "oauth_refresh_token")
                access_token = self.config_manager.retrieve_credential("google", "oauth_access_token")
    ...
    def get_credential_status(self, service: str, credential_type: str) -> Optional[CredentialStatus]:
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        healthy_credentials = sum(1 for status in self.credential_statuses.values() if status.healthy)
        ...
        return {
            ...
            "health_percentage": (healthy_credentials / total_credentials * 100) if total_credentials > 0 else 0,
            "expiring_soon": expiring_soon,
            "failed_credentials": failed_credentials,
            "last_check": max((status.last_checked for status in self.credential_statuses.values()), default=None),
        ...
        try:
            ...
            if status_file.exists():
                ...
                for status_key, status_data in data.items():
                    ...
                    for date_field in ["last_checked", "last_success", "expires_at", "next_check"]:
                    ...
                    self.credential_statuses[status_key] = CredentialStatus(**status_data)
                
                self.logger.info(f"Loaded credential status for {len(self.credential_statuses)} credentials")
...
class CredentialNotificationManager:
    ...
    def on_credential_expiring(self, service: str, credential_type: str, days_until_expiry: int):
    ...
    def on_credentials_refreshed(self, service: str, credential_type: str):
        ...
        message = f"Credential {service}:{credential_type} has been automatically refreshed"
