...
from ..utils.exceptions import (
    ExecutiveSummaryToolError, JiraIntegrationError, 
    GeminiIntegrationError, GoogleDocsIntegrationError
)
...
class WorkflowOrchestrator:
    ...
    async def execute_workflow(
        ...
        try:
            ...
            document_info = await self._execute_stage(
                "create_document", result, summary, document_title, folder_id, share_email
            )
            if self.is_cancelled:
                return self._handle_cancellation(result)
    ...
    async def _execute_stage(self, stage_name: str, result: WorkflowResult, *args, **kwargs):
        ...
        self._update_progress(f"Stage {stage_number}: {stage_name.replace('_', ' ').title()}")
    ...
    def get_activity_summary(self, activity_data: List[Dict[str, Any]]) -> Dict[str, Any]:
