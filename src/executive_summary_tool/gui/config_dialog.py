...
from PySide6.QtWidgets import (
    ...
    QCheckBox, QComboBox, QFormLayout, QGroupBox, QFileDialog,
...
class ConfigDialog(QDialog):
    ...
    def init_ui(self):
        ...
        button_box.button(QDialogButtonBox.Apply).clicked.connect(self.apply_configuration)
    ...
    def create_jira_tab(self):
        ...
        self.default_query_edit.setPlaceholderText("project = PROJECT AND updated >= -1w")
    ...
    def create_google_tab(self):
        ...
        self.service_account_path_edit.setPlaceholderText("Path to service account JSON file")
    ...
    def create_app_tab(self):
        ...
        self.telemetry_check = QCheckBox("Enable telemetry (helps improve the application)")
    ...
    def load_configuration(self):
        ...
        try:
            ...
            oauth_secret = self.config_manager.retrieve_credential("google", "oauth_client_secret")
            if oauth_secret:
    ...
    def save_configuration(self):
        ...
        try:
            ...
            if self.jira_token_edit.text():
                self.config_manager.store_credential("jira", "api_token", self.jira_token_edit.text())
            
            if self.gemini_api_key_edit.text():
                self.config_manager.store_credential("ai", "gemini_api_key", self.gemini_api_key_edit.text())
            
            if self.oauth_client_secret_edit.text():
                self.config_manager.store_credential("google", "oauth_client_secret", self.oauth_client_secret_edit.text())
    ...
    def test_connections(self):
        ...
        QMessageBox.information(self, "Connection Test", "Connection testing feature coming soon.")
    
    def apply_configuration(self):
        """Apply configuration without closing dialog."""
        try:
            self.save_configuration()
            QMessageBox.information(self, "Configuration Applied", "Configuration saved successfully.")
        except Exception as e:
    ...
    def validate_configuration(self) -> bool:
        ...
        try:
            ...
            if not self.jira_username_edit.text():
                QMessageBox.warning(self, "Validation Error", "Jira username is required.")
                return False
            
            if not self.jira_token_edit.text() and not self.config_manager.retrieve_credential("jira", "api_token"):
                QMessageBox.warning(self, "Validation Error", "Jira API token is required.")
                return False
            
            if not self.gemini_api_key_edit.text() and not self.config_manager.retrieve_credential("ai", "gemini_api_key"):
                QMessageBox.warning(self, "Validation Error", "Gemini API key is required.")
                return False
